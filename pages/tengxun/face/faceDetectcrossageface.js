...
Page({
  ...
  detectcrossageface: function (base64) {
    ...
    map.set("source_image", encodeURIComponent(this.data.filesBase64[0]))
    map.set("target_image", encodeURIComponent(this.data.filesBase64[1]))

    var md5Param = util.signTengxunAI(map)
    this.requestData.sign = md5Param
    var that = this
    http.req('https://api.ai.qq.com', '/fcgi-bin/face/face_detectcrossageface', {
      app_id: this.requestData.app_id,
      time_stamp: this.requestData.time_stamp,
      nonce_str: this.requestData.nonce_str,
      source_image: this.data.filesBase64[0],
      target_image: this.data.filesBase64[1],
      sign: md5Param
    }, function (res) {
      wx.hideLoading()
